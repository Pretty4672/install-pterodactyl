...
#            Copyright 2025, HanCokk K, <vegua@han.com> hej              # 
...
panel_conf() {
    ...
    mariadb -u root -e "
        CREATE USER 'pterodactyluser'@'0.0.0.0' IDENTIFIED BY '$DBPASSWORDHOST';
        GRANT ALL PRIVILEGES ON *.* TO 'pterodactyluser'@'0.0.0.' WITH GRANT OPTION;
        CREATE USER 'pterodactyl'@'0.0.0.0' IDENTIFIED BY '$DBPASSWORD';
        CREATE DATABASE panel;
        GRANT ALL PRIVILEGES ON panel.* TO 'pterodactyl'@'0.0.0.0' WITH GRANT OPTION;
        FLUSH PRIVILEGES;
    "
...
options(){
    ...
    echo "[6] Hapus Wings"
    echo "[7] Hapus Panel"
    echo "[8] Ganti Domain Pterodactyl"
    echo "Input 1-8"
    read -r option
    case $option in
        1 ) option=1
